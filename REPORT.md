...
# 第五部分 · 记忆读取回流（场景 32，源码 + 实证）

## 18. 记忆是怎么"读回来"的

源码定性（`rust-v0.137.0`）：
- 记忆**不是**启动时自动注入 prompt 块，而是**模型按需读取** `~/.codex/memories/MEMORY.md` / `memory_summary.md`
  （经 `memories` namespace 工具的 `read`/`search`/`list` 子工具，或 shell）。`codex-rs/memories/read/src/usage.rs:44` 按
  `memories/MEMORY.md`、`memories/memory_summary.md` 路径归类读取来源。
- `use_memories` 的作用（`core/src/config/mod.rs:3366`）：仅把 `memories_root` 加入沙箱**可读根**
  （`helper_readable_roots`），并不注入文本。
- 记忆**生成**期望 `{rollout_summary, rollout_slug, raw_memory}`，三者全空则 no-op（`memories/write/src/phase1.rs:258`）
  ——所以 mock 回 `pong` 时不落盘任何记忆。

实证（mock 回 `exec_command cat ~/.codex/memories/MEMORY.md`，植入含 `SENTINEL_MEMREAD_42` 的真实 MEMORY.md）：
- ✅ **读取回流成立**（`logs/32a-memread-use-on`）：模型读到 MEMORY.md，`SENTINEL_MEMREAD_42` 进入
  `function_call_output` → 即回流进下一请求的 `input`，成为模型上下文。
- ⚠️ **use_memories 开/关在 `-s read-only` 下无可观测差异**（`logs/32b-memread-use-off` 的 cat 同样成功）：
  read-only 沙箱本就允许读取任意文件，`use_memories→可读根` 的门控只在**限制读取的沙箱**下才显现。
  即：在 read-only 下，记忆读取不受 `use_memories` 限制——这修正了"use_memories 决定能否读记忆"的直觉。

## 19. 对推理引擎的启示

记忆回流是**模型主动发起的工具读**（function_call: 读 MEMORY.md → function_call_output 注入上下文），
不是协议层的自动注入。⇒ 推理引擎在请求层看到的"记忆"表现为**普通的 tool-read 往返**（exec_command / memories 工具），
其 `function_call_output` 里携带记忆正文；可据此识别"这一轮在消费记忆"，但无法在请求 body 里看到独立的记忆字段。
